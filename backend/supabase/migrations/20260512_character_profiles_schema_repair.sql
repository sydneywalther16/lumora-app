...
set
  ...
  wardrobe_tendencies = coalesce(nullif(wardrobe_tendencies, ''), style_preferences->>'wardrobeTendencies', style_preferences->>'fashionStyle', ''),
  emotional_tendencies = coalesce(nullif(emotional_tendencies, ''), style_preferences->>'emotionalTendencies', style_preferences->>'characterVibe', ''),
  ...
  continuity_state = case
    when continuity_state = '{}'::jsonb then jsonb_strip_nulls(jsonb_build_object(
      ...
      'wardrobe', coalesce(nullif(wardrobe_tendencies, ''), style_preferences->>'wardrobeTendencies', style_preferences->>'fashionStyle'),
      'emotionalTone', coalesce(nullif(emotional_tendencies, ''), style_preferences->>'emotionalTendencies', style_preferences->>'characterVibe'),
...
do $$
begin
  if to_regclass('public.continuity_memory_states') is not null then
    execute 'grant select, insert, update, delete on table continuity_memory_states to authenticated, service_role';
  end if;
end $$;

alter table character_profiles enable row level security;

drop policy if exists "character_profiles_own_all" on character_profiles;
