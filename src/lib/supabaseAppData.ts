...
function mapCharacterRow(row: DbRow): CharacterProfile {
  ...
  return {
    ...
    verificationConsentPresent: Boolean(nullableString(row.verification_consent_at)),
    verificationStatus: nullableString(row.verification_status),
    verificationPrompt: nullableString(row.verification_prompt),
    verificationLastTestedAt: nullableString(row.verification_last_tested_at),
    videoReferenceRouteStatus: nullableString(row.video_reference_route_status),
    videoReferenceProvider: nullableString(row.video_reference_provider),
...
function mapProjectRow(row: DbRow): StudioProject {
  ...
  return {
    ...
    stage2ProviderModel: diagnosticString('stage2ProviderModel'),
    stage2ProviderRouteType: diagnosticString('stage2ProviderRouteType'),
    rawReferenceVisualInputsSentToStage2: diagnosticBoolean('rawReferenceVisualInputsSentToStage2'),
